...
class saleControlerHelper {
  ...
  static async processSale(req, res, next) {
    const { errors, isValid } = _sales2.default.validateSalesInput(req.body);
    ...
    const promises = order.map(async singleorder => {
      ...
      const productqueryvalue = [productId];
      const response = await _db2.default.query(text, productqueryvalue);
      return response;
    });
  ...
  static updateProduct(req, res, next) {
    ...
    order.map(singleord => {
      ...
      const values = [productId, Number(singleorder.latestquantitytobeupdatedindb), new Date()];
      _db2.default.query(text, values).then(() => {}).catch(() => {});
...
exports.default = saleControlerHelper;